...
# ── Workstream A/B imports — best-effort ─────────────────────────────────────
#
# Workstream A (`providers_anthropic_convert.py`) and B
# (`providers_anthropic_stream.py`) ship the converters this module depends on.
# The orchestrator merges all three files together at integration time, but
# during parallel development the imports may not resolve. Keep the stubs in
# place so this module imports cleanly and tests can monkey-patch the
# converters directly.
#
# ORCHESTRATOR: remove this try/except stub block once A and B are on main.
try:
    from providers_anthropic_convert import (  # type: ignore[import-not-found]
        from_anthropic_response,
        to_anthropic_request,
    )
    from providers_anthropic_stream import (  # type: ignore[import-not-found]
        reassemble_anthropic_stream,
    )
except ImportError:  # pragma: no cover — stubs kept for parallel-dev safety
    def to_anthropic_request(**kw: Any) -> dict[str, Any]:  # type: ignore[misc]
        return kw

    def from_anthropic_response(resp: Any) -> Any:  # type: ignore[misc]
        return resp

    def reassemble_anthropic_stream(events: Iterable[dict]) -> Iterator[Any]:  # type: ignore[misc]
        yield from ()
