...
# Workstream A owns _map_stop_reason. While A and B are developed in parallel,
# fall back to an inline stub so this module is self-contained and importable
# even if A is not yet merged. The orchestrator (final integrator) will delete
# the stub block at merge time, leaving only the `from ... import` line.
try:
    from providers_anthropic_convert import _map_stop_reason
except ImportError:  # pragma: no cover - merge-time stub only
    def _map_stop_reason(anthropic_stop: Optional[str]) -> Optional[str]:
